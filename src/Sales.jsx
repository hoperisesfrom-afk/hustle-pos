    ...
    <div>
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        {['list', 'new'].map(v => (
          ...
          }}>{v === 'list' ? `Sales (${sales.length})` : '+ New Sale'}</button>
        ))}
      </div>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: 20 }}>
          {availableStock.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '30px 0' }}>
              ...
              <button onClick={() => setTab('inventory')} style={{ background: 'var(--gold)', color: '#0a0a0a', border: 'none', borderRadius: 8, padding: '10px 20px', fontSize: 13, fontFamily: 'var(--mono)', cursor: 'pointer' }}>Go to Stock →</button>
            </div>
          ) : (
            <>
              <div style={{ marginBottom: 14 }}>
                <label style={lbl}>Customer name *</label>
                <input value={form.customerName} onChange={e => set('customerName', e.target.value)} placeholder="e.g. Wanjiku" style={inp} />
              </div>
              <div style={{ marginBottom: 14 }}>
              ...
              {selectedItem && (
                <div style={{ background: 'var(--gold-bg)', border: '1px solid var(--gold-dim)', borderRadius: 8, padding: '12px 14px', marginBottom: 14 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 14 }}>
                <div>
                  ...
                  <input type="number" value={form.paid} onChange={e => set('paid', e.target.value)} placeholder="0" min="0" style={inp} />
                </div>
                <div>
      ...
      {view === 'list' && (
        <>
                ...
                return (
                  <div key={s.id} style={{ background: 'var(--surface)', border: '1px solid rgba(224,92,92,0.3)', borderRadius: 10, padding: '14px', marginBottom: 8 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      {paying === s.id ? (
                        <>
                          <input type="number" value={extraPay} onChange={e => setExtraPay(e.target.value)} placeholder="Amount received" style={{ ...inp, flex: 1, padding: '7px 10px' }} />
                          <button onClick={handleMarkPaid} style={{ background: 'var(--green)', color: '#0a0a0a', border: 'none', borderRadius: 6, padding: '7px 14px', fontSize: 12, fontFamily: 'var(--mono)', cursor: 'pointer' }}>✓</button>
                          <button onClick={() => setPaying(null)} style={{ background: 'var(--surface2)', color: 'var(--text-muted)', border: '1px solid var(--border)', borderRadius: 6, padding: '7px 14px', fontSize: 12, fontFamily: 'var(--mono)', cursor: 'pointer' }}>✕</button>
                        </>
                      ) : (
                        <>
                          <button onClick={() => { setPaying(s.id); setExtraPay(''); }} style={{ fontSize: 11, padding: '6px 12px', background: 'var(--green-bg)', color: 'var(--green)', border: '1px solid rgba(76,175,125,0.2)', borderRadius: 6, cursor: 'pointer' }}>+ Record payment</button>
                          <button onClick={() => deleteSale(s.id)} style={{ fontSize: 11, padding: '6px 12px', background: 'var(--red-bg)', color: 'var(--red)', border: '1px solid rgba(224,92,92,0.2)', borderRadius: 6, cursor: 'pointer' }}>Delete</button>
                        </>
                      )}
                    </div>
                  </div>
          ...
          {settled.length > 0 && (
            <div>
              <div style={{ fontSize: 11, color: 'var(--green)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 8 }}>Settled ({settled.length})</div>
              {settled.map(s => (
