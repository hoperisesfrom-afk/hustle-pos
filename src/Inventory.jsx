...
const empty = { name: '', category: 'Rings', buyingPrice: '', sellingPrice: '', qty: '' };
...
export default function Inventory({ inventory, addInventoryItem, deleteInventoryItem }) {
  ...
  const submit = () => {
    ...
    setError('');
    addInventoryItem({ name: form.name.trim(), category: form.category, buyingPrice: b, sellingPrice: s, qty: q });
    setForm(empty);
    setView('list');
  };
  ...
  const totalInvested = inventory.reduce((a, i) => a + i.buyingPrice * i.qty, 0);
  const totalValue = inventory.reduce((a, i) => a + i.sellingPrice * i.remaining, 0);
    ...
    <div>
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        {['list', 'add'].map(v => (
          <button key={v} onClick={() => setView(v)} style={{
            ...
            {v === 'list' ? `Stock (${inventory.length})` : '+ Add Stock'}
          </button>
        ))}
      </div>
      ...
      {view === 'list' && (
        <>
          ...
          {inventory.length === 0 ? (
            <div style={empty_state}>No stock yet — tap "+ Add Stock" to begin</div>
          ) : (
            inventory.map(item => {
              ...
              return (
                <div key={item.id} style={{
                  background: 'var(--surface)', border: `1px solid ${out ? 'var(--red)' : low ? '#8a6d2a' : 'var(--border)'}`,
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginTop: 12 }}>
                    <div><div style={lbl}>Bought at</div><div style={val}>Ksh {item.buyingPrice.toLocaleString('en-KE')}</div></div>
                    <div><div style={lbl}>Selling at</div><div style={val}>Ksh {item.sellingPrice.toLocaleString('en-KE')}</div></div>
                    <div><div style={lbl}>Margin</div><div style={{ ...val, color: 'var(--green)' }}>Ksh {(item.sellingPrice - item.buyingPrice).toLocaleString('en-KE')}</div></div>
                  </div>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: 20 }}>
          <div style={{ marginBottom: 14 }}>
            ...
            <input value={form.name} onChange={e => set('name', e.target.value)}
              placeholder="e.g. Gold ring" style={inp} />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 14 }}>
            ...
            <div>
              ...
              <input type="number" value={form.qty} onChange={e => set('qty', e.target.value)}
                placeholder="0" min="1" style={inp} />
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 14 }}>
            <div>
              <label style={lbl}>Buying price (Ksh)</label>
              <input type="number" value={form.buyingPrice} onChange={e => set('buyingPrice', e.target.value)}
...
const inp = { width: '100%', padding: '10px 12px', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, color: 'var(--text)', fontSize: 13, fontFamily: 'var(--mono)', outline: 'none' };
const empty_state = { textAlign: 'center', padding: '40px 20px', border: '1px dashed var(--border)', borderRadius: 10, color: 'var(--text-muted)', fontSize: 13 };
